...
class Selfpatch(object):
    ...
    def _extract_patches(self, img, patch_size, stride):
        ...
        i_1, i_2, i_3, i_4, i_5 , i_6= input_windows.size(0), input_windows.size(1), input_windows.size(2), input_windows.size(3), input_windows.size(4), input_windows.size(5)
        input_windows = input_windows.permute(0,2,3,1,4,5).contiguous().view(i_1,i_3*i_4, i_2, i_5, i_6)
...
class BASE(nn.Module):
    ...
    def forward(self, x):
        ...
        b, c, h, w = out_32.size()
        gus = self.gus.float()
        gus_out = out_32.unsqueeze(1).expand(b, h * w, c, h, w)
        gus_out = gus.to(gus_out.device) * gus_out
        gus_out = torch.sum(gus_out, -1)
        gus_out = torch.sum(gus_out, -1)
        gus_out = gus_out.view(b, c, h, w)
        csa2_in = F.sigmoid(out_32)
        csa2_f = torch.nn.functional.pad(csa2_in, (1, 1, 1, 1))
        csa2_ff = torch.nn.functional.pad(out_32, (1, 1, 1, 1))
        csa2_fff, csa2_f, csa2_conv = Nonparm.buildAutoencoder(csa2_f, csa2_in, csa2_ff, 3, 1)
        csa2_conv = csa2_conv.expand_as(csa2_f)
        csa_a = csa2_conv * csa2_f
        csa_a = torch.mean(csa_a, 2)
        
        b, a_c, a_h, a_w = csa_a.size()
        csa_a = csa_a.view(b, a_c, -1)
        csa_a = F.softmax(csa_a, dim=1)
        csa_a = csa_a.view(b, a_c, 1, a_h, a_h)
        out = csa_a * csa2_fff
        out = torch.sum(out, -1)
        out = torch.sum(out, -1)
        out_csa = out.view(b, c, h, w)
...
class PCconv(nn.Module):
    ...
    def forward(self, input, mask):
        ...
        '''
        x_final = torch.zeros([b,512,32,32],dtype=torch.float).cuda()
        for i in range(0,b):
            x_final[i] = self.base(x_cat_fuse[i,:].unsqueeze(0))
		'''
        '''
        x_final = torch.tensor([])
        for i in range(0,b):
            x_final = torch.cat([x_final,self.base(x_cat_fuse[i,:].unsqueeze(0)).to(x_final.device)], 0)
        '''
