...
def generator_loss(discr_fake_pred: torch.Tensor, mask=None):
    ...
    if (mask_as_fake_target and extra_mask_weight_for_gen > 0) or not use_unmasked_for_gen:
        mask = interpolate_mask(mask, discr_fake_pred.shape[-2:], allow_scale_mask, mask_scale_mode)
        if not use_unmasked_for_gen:
            fake_loss = fake_loss * mask
        else:
            pixel_weights = 1 + mask * extra_mask_weight_for_gen
            fake_loss = fake_loss * pixel_weights

    return fake_loss.mean() * weight
...
def discriminator_fake_loss(discr_fake_pred: torch.Tensor, mask=None):
    ...
    if not use_unmasked_for_discr or mask_as_fake_target:
        # == if masked region should be treated differently
        mask = interpolate_mask(mask, discr_fake_pred.shape[-2:], allow_scale_mask, mask_scale_mode)
        # use_unmasked_for_discr=False only makes sense for fakes;
        # for reals there is no difference beetween two regions
        fake_loss = fake_loss * mask
        if mask_as_fake_target:
